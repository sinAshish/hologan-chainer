...
class Block(chainer.Chain):
    def __init__(self, in_channels, out_channels, hidden_channels=None, ksize=3, pad=1,
        ...
        with self.init_scope():
            ...
            self.inst_norm1 = InstanceNormalization(hidden_channels)
            self.inst_norm2 = InstanceNormalization(out_channels)
            if self.learnable_sc:
                self.c_sc = SNConvolution2D(in_channels, out_channels, ksize=1, pad=0, initialW=initializer_sc)
                self.inst_norm3 = InstanceNormalization(out_channels)
...
class OptimizedBlock(chainer.Chain):
    def __init__(self, in_channels, out_channels, ksize=3, pad=1, activation=F.relu):
        ...
        with self.init_scope():
            ...
            self.c_sc = SNConvolution2D(in_channels, out_channels, ksize=1, pad=0, initialW=initializer_sc)
            self.inst_norm1 = InstanceNormalization(out_channels)
